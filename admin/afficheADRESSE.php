...
return '<form method="POST" action="../accueil_adh.php?page=supprimer">
    <input type=hidden name="id_adresse" value="'.$login.'">
...
<table>
    ...
    try {
        ...
        foreach ($db->query('SELECT * from ADRESSE') as $ligne) {
            //Affichage du tableau HTML des personnes
            echo '<tr> 
                <td>' . $ligne['id_adresse'] . '</td>
                <td>' . $ligne['num_voie'] . '</td>
                <td>' . $ligne['indicateur_eventuel_repetition'] . '</td>
                <td>' . $ligne['type_de_voie'] . '</td>
                <td>' . $ligne['nom_voie'] . '</td>
                <td>' . $ligne['code_postal'] . '</td><td>';
                echo formSuppression($ligne['id_adresse']);
                echo '</td></tr>';
        }
    $db=null;
